...
function setup() {
 ...
 console.log(message);
  ...
  trex = createSprite(150,height-89,40,80);
  trex.addAnimation("running", trex_running);
  trex.addAnimation("collided", trex_collided);
  trex.scale = 0.6;
  
  ground = createSprite(width-70,height-60,windowWidth+200,20);
  ...
  restart = createSprite(width/2,gameOver.y+45,100,100);
...
function draw() {
  ...
  if(gameState==END && keyDown("space") || touches.hi){
    reset();  
  }
  drawSprites();
}
...
function spawnObstacles(){
 if (frameCount % 90 === 0){
   var obstacle = createSprite(width+20,height-72,10,40);
